...
var BloomNode = class extends TempNode {
  ...
  constructor(inputNode, strength = 1, radius = 0, threshold3 = 0) {
    super("vec4");
    this.inputNode = inputNode;
    this.strength = uniform2(strength);
    this.radius = uniform2(radius);
    this.threshold = uniform2(threshold3);
  ...
  updateBefore(frame) {
    ...
    _quadMesh2.material = this._highPassFilterMaterial;
    _quadMesh2.render(renderer);
    let inputRenderTarget = this._renderTargetBright;
    for (let i = 0; i < this._nMips; i++) {
      _quadMesh2.material = this._separableBlurMaterials[i];
  ...
  _getSeperableBlurMaterial(builder, kernelRadius) {
    ...
    const colorTexture = texture2();
    const gaussianCoefficients = uniformArray2(coefficients);
    const invSize = uniform2(new Vector2());
    const direction2 = uniform2(new Vector2(0.5, 0.5));
    const uvNode = uv2();
    const sampleTexel = (uv3) => colorTexture.uv(uv3);
    const seperableBlurPass = Fn2(() => {
      ...
      Loop2({ start: int2(1), end: int2(kernelRadius), type: "int", condition: "<" }, ({ i }) => {
        ...
        diffuseSum.addAssign(add2(sample1, sample2).mul(w));
        weightSum.addAssign(float2(2).mul(w));
      });
      return vec42(diffuseSum.div(weightSum), 1);
    });
...
var bloom = (node, strength, radius, threshold3) => nodeObject2(new BloomNode(nodeObject2(node), strength, radius, threshold3));
